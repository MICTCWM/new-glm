...
func RelaySwapFace(c *gin.Context, info *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	defer func() {
		if mjResp.StatusCode == 200 && mjResp.Response.Code == 1 {
			...
			logContent = service.PrefixBillingLogContent(info.BillingSource, logContent)
			...
			model.UpdateChannelCallCount(info.ChannelId, 1)
		}
	}()
...
func RelayMidjourneySubmit(c *gin.Context, relayInfo *relaycommon.RelayInfo) *dto.MidjourneyResponse {
	...
	defer func() {
		if consumeQuota && midjResponseWithStatus.StatusCode == 200 {
			...
			logContent = service.PrefixBillingLogContent(relayInfo.BillingSource, logContent)
