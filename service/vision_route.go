...
// 条件：模型在不支持视觉的列表中（默认所有模型都支持视觉）+ 已配置可路由的视觉模型 + 请求含图片
func ShouldVisionRoute(relayFormat types.RelayFormat, modelName string, request dto.Request) bool {
	if !operation_setting.IsModelNoImage(modelName) {
		common.SysLog(fmt.Sprintf("vision route skipped: model %q not in no-vision list", modelName))
		return false
	}
	if len(operation_setting.VisionRouteModels) == 0 {
		common.SysLog("vision route skipped: no vision route models configured")
		return false
	}
	if request == nil {
...
// ProcessVisionRoute 执行视觉路由：复用缓存的图片描述，仅将未命中的图片发送给视觉模型，
// 然后替换原请求中的图片位置。如果视觉模型调用失败，返回错误，由调用方进行降级处理
...
func resolveVisionImageDescriptions(c *gin.Context, relayInfo *relaycommon.RelayInfo, imageContents []dto.MediaContent) ([]string, int, int, error) {
	...
	for _, imageContent := range imageContents {
		...
		if key == "" {
			description, err := callVisionModelForImageDescription(c, relayInfo, []dto.MediaContent{imageContent})
			if err != nil {
				return nil, 0, 0, err
			}
			descriptions = append(descriptions, description)
...
// callVisionModelForImageDescription 调用配置的视觉模型描述图片
// 内部调用不扣费，扣费在目标模型请求结算时统一处理
func callVisionModelForImageDescription(c *gin.Context, relayInfo *relaycommon.RelayInfo, imageContents []dto.MediaContent) (string, error) {
	visionModels := operation_setting.VisionRouteModels
	if len(visionModels) == 0 {
		return "", fmt.Errorf("no vision route models configured")
	}
	...
	// 遍历已配置的视觉模型列表，尝试获取可用渠道
	var lastErr error
	for _, visionModel := range visionModels {
		channel, err := model.GetRandomSatisfiedChannel(group, visionModel, 0, nil, relayInfo.UserId)
		if err != nil {
			lastErr = fmt.Errorf("failed to get channel for vision model %s: %w", visionModel, err)
			continue
		}
		if channel == nil {
			lastErr = fmt.Errorf("no available channel for vision model %s in group %s", visionModel, group)
			continue
		}
		...
		if keyErr != nil {
			lastErr = fmt.Errorf("no available key for vision model %s: %v", visionModel, keyErr)
			continue
		}
		if key == "" {
			lastErr = fmt.Errorf("key is empty for vision model %s", visionModel)
			continue
		}

		logger.LogInfo(c, fmt.Sprintf("vision route: calling vision model %s channel %d for image description", visionModel, channel.Id))
		...
		reqBody, err := common.Marshal(visionReq)
		if err != nil {
		...
		if baseURL == "" {
			return "", fmt.Errorf("vision channel base url is empty")
		}
		...
		if err != nil {
			logger.LogError(c, fmt.Sprintf("vision route: call to %s failed: %v", visionModel, err))
			lastErr = fmt.Errorf("failed to call vision model %s: %w", visionModel, err)
			continue
		}
		...
		if resp.StatusCode != http.StatusOK {
			...
			logger.LogError(c, fmt.Sprintf("vision route: %s returned non-200 status: %d, body: %s", visionModel, resp.StatusCode, string(body)))
			lastErr = fmt.Errorf("vision model %s returned non-200 status: %d, body: %s", visionModel, resp.StatusCode, string(body))
			continue
		}
		...
		var visionResp dto.OpenAITextResponse
		if err := common.Unmarshal(body, &visionResp); err != nil {
			return "", fmt.Errorf("failed to unmarshal vision response: %w", err)
		}

		if len(visionResp.Choices) == 0 {
			return "", fmt.Errorf("vision model %s returned no choices", visionModel)
		}

		description := visionResp.Choices[0].Message.StringContent()
		if description == "" {
			return "", fmt.Errorf("vision model %s returned empty description", visionModel)
		}

		elapsed := time.Since(start)
		logger.LogInfo(c, fmt.Sprintf("vision route: %s responded in %s, description length: %d", visionModel, elapsed, len(description)))

		return description, nil
	}
	return "", fmt.Errorf("all vision models exhausted: %w", lastErr)
}
